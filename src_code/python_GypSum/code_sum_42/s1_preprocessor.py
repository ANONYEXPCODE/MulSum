...
from my_lib.util.code_parser.code_tokenizer import CompoundWordSplitter,tokenize_code_str
import codecs
import os
...
from my_lib.util.code_parser.code_parser import SitParser
from my_lib.util.code_parser.code_tokenizer import tokenize_code_str
...
def _truncate_ast_by_code(parser,start_code_token_id,end_code_token_id,max_graph_size=np.inf,max_nleaf_num=np.inf,max_code_len=np.inf,renew_pos=True,):    
    ...
    if len(keep_ast_node_ids)>max_graph_size or len(set(keep_ast_node_ids)&set(code_token_ids))>max_code_len or len(keep_ast_node_ids)-len(set(keep_ast_node_ids)&set(code_token_ids))>max_nleaf_num:
        ...
        keep_ast_node_ids=[]
        keep_code_node_ids=[]
        while node_id_stack and len(keep_ast_node_ids)<max_graph_size and len(set(keep_ast_node_ids)&set(code_token_ids))<max_code_len and len(keep_ast_node_ids)-len(set(keep_ast_node_ids)&set(code_token_ids))<max_nleaf_num:
    ...
    if len(keep_ast_node_ids)<len(_parser_dict['ast_nodes']):
        ...
        # print(code_len,parser_dict['code_token_edges'].shape)
...
def tokenize_raw_data(raw_data_path, token_data_path, rev_dic_path,
    ...
    for item in pbar:
        ...
        parser_dict=_truncate_ast_by_code(parser,0,min(len(parser.code_tokens),max_code_len),max_graph_size=max_graph_size,max_code_len=max_code_len,max_nleaf_num=max_nleaf_num,renew_pos=False)
        
        ast_nodes=[token if token not in parser.digits and not token.isdigit() else '<number>' for token in parser_dict['ast_nodes']]
        assert parser_dict['ast_edges'].dtype==np.int64
        assert parser_dict['ast_sibling_edges'].dtype==np.int64
        
        item['graph'] = {'nodes': str(ast_nodes),
                       'base_edges': str(parser_dict['ast_edges'].tolist()),
                       'sibling_edges':str(parser_dict['ast_sibling_edges'].tolist()),
                       'dfg_edges':str(parser_dict['DFG_edges'].tolist()),
                       'code_edges':str(parser_dict['code_token_edges'].tolist()),
...
def build_w2i2w(train_token_data_path,
    ...
    for item in tqdm(token_data):
        ...
        max_graph_size = max(max_graph_size, len(list(eval(item['graph']['nodes']))))
...
def build_avail_data(token_data_path,
    ...
    for i,item in enumerate(pbar):
        ...
        nleaf_nodes,code_nodes=[],[]
        nleaf_id2id,code_id2id=dict(),dict()
        for j,node in enumerate(graph_nodes):
            if j in code_prev2next_edges:
                code_id2id[j]=len(code_nodes)
                code_nodes.append(node)
            else:
                nleaf_id2id[j]=len(nleaf_nodes)
                nleaf_nodes.append(node)
        
        nleaf_father_nleaf_base_edges=[]
        code_father_nleaf_base_edges=[]
        nleaf_prev_nleaf_sibling_edges=[]
        nleaf_prev_code_sibling_edges=[]
        code_prev_code_sibling_edges=[]
        code_prev_nleaf_sibling_edges=[]
        nleaf_prev_nleaf_dfg_edges=[]
        code_prev_code_code_edges=[]
        for j in range(base_child2father_edges.shape[1]):
            child_id,father_id=base_child2father_edges[0,j],base_child2father_edges[1,j]
            assert father_id not in code_prev2next_edges
            if child_id not in code_prev2next_edges:
                nleaf_father_nleaf_base_edges.append([[nleaf_id2id[child_id],nleaf_id2id[father_id]]])
            else:
                code_father_nleaf_base_edges.append([[code_id2id[child_id],nleaf_id2id[father_id]]])
        for j in range(sibling_prev2next_edges.shape[1]):
            child_id,father_id=sibling_prev2next_edges[0,j],sibling_prev2next_edges[1,j]
            if child_id not in code_prev2next_edges:
                if father_id not in code_prev2next_edges:
                    nleaf_prev_nleaf_sibling_edges.append([[nleaf_id2id[child_id],nleaf_id2id[father_id]]])
                else:
                    nleaf_prev_code_sibling_edges.append([[nleaf_id2id[child_id],code_id2id[father_id]]])
            else:
                if father_id in code_prev2next_edges:
                    code_prev_code_sibling_edges.append([[code_id2id[child_id],code_id2id[father_id]]])
                else:
                    code_prev_nleaf_sibling_edges.append([[code_id2id[child_id],nleaf_id2id[father_id]]])
        for j in range(dfg_prev2next_edges.shape[1]):
            child_id,father_id=dfg_prev2next_edges[0,j],dfg_prev2next_edges[1,j]
            assert child_id not in code_prev2next_edges and father_id not in code_prev2next_edges
            nleaf_prev_nleaf_dfg_edges.append([[nleaf_id2id[child_id],nleaf_id2id[father_id]]])
        for j in range(code_prev2next_edges.shape[1]):
            child_id,father_id=code_prev2next_edges[0,j],code_prev2next_edges[1,j]
            assert child_id in code_prev2next_edges and father_id in code_prev2next_edges
            code_prev_code_code_edges.append([[code_id2id[child_id],code_id2id[father_id]]])
        
        nleaf_father_nleaf_base_edges=np.concatenate(nleaf_father_nleaf_base_edges,axis=0).T
        nleaf_child_nleaf_base_edges=np.array([nleaf_father_nleaf_base_edges[1,:],nleaf_father_nleaf_base_edges[0,:]])
        code_father_nleaf_base_edges=np.concatenate(code_father_nleaf_base_edges,axis=0).T
        nleaf_child_code_base_edges=np.array([code_father_nleaf_base_edges[1,:],code_father_nleaf_base_edges[0,:]])
        
        if len(nleaf_prev_nleaf_sibling_edges)>0:
            nleaf_prev_nleaf_sibling_edges=np.concatenate(nleaf_prev_nleaf_sibling_edges,axis=0).T
            nleaf_next_nleaf_sibling_edges=np.array([nleaf_prev_nleaf_sibling_edges[1,:],nleaf_prev_nleaf_sibling_edges[0,:]])
        else:
            nleaf_prev_nleaf_sibling_edges=np.array([])
            nleaf_next_nleaf_sibling_edges=np.array([])
        if len(nleaf_prev_code_sibling_edges)>0:
            nleaf_prev_code_sibling_edges=np.concatenate(nleaf_prev_code_sibling_edges,axis=0).T
            code_next_nleaf_sibling_edges=np.array([nleaf_prev_code_sibling_edges[1,:],nleaf_prev_code_sibling_edges[0,:]])
        else:
            nleaf_prev_code_sibling_edges=np.array([])
            code_next_nleaf_sibling_edges=np.array([])
        if len(code_prev_code_sibling_edges)>0:
            code_prev_code_sibling_edges=np.concatenate(code_prev_code_sibling_edges,axis=0).T
            code_next_code_sibling_edges=np.array([code_prev_code_sibling_edges[1,:],code_prev_code_sibling_edges[0,:]])
        else:
            code_prev_code_sibling_edges=np.array([])
            code_next_code_sibling_edges=np.array([])
        if len(code_prev_nleaf_sibling_edges)>0:
            code_prev_nleaf_sibling_edges=np.concatenate(code_prev_nleaf_sibling_edges,axis=0).T
            nleaf_next_code_sibling_edges=np.array([code_prev_nleaf_sibling_edges[1,:],code_prev_nleaf_sibling_edges[0,:]])
        else:
            code_prev_nleaf_sibling_edges=np.array([])
            nleaf_next_code_sibling_edges=np.array([])

        if len(nleaf_prev_nleaf_dfg_edges)>0:
            nleaf_prev_nleaf_dfg_edges=np.concatenate(nleaf_prev_nleaf_dfg_edges,axis=0).T
            nleaf_next_nleaf_dfg_edges=np.array([nleaf_prev_nleaf_dfg_edges[1,:],nleaf_prev_nleaf_dfg_edges[0,:]])
        else:
            nleaf_prev_nleaf_dfg_edges=np.array([])
            nleaf_next_nleaf_dfg_edges=np.array([])
        
        code_prev_code_code_edges=np.concatenate(code_prev_code_code_edges,axis=0).T
        code_next_code_code_edges=np.array([code_prev_code_code_edges[1,:],code_prev_code_code_edges[0,:]])

        text_tokens=item['text'].split()

        ex_text_w2i, ex_text_i2w = get_ex_tgt_dict(code_nodes, text_w2i)
        code2text_map_ids = get_src2tgt_map_ids(code_nodes, text_w2i, ex_text_w2i)
        ...
        if unk_aliased:
            ...
            nleaf_nodes = [node if node not in unk_aliases else '<unk-alias-{}>'.format(unk_aliases.index(node)) for node in nleaf_nodes]
            code_nodes = [node if node not in unk_aliases else '<unk-alias-{}>'.format(unk_aliases.index(node)) for node in code_nodes]

        nleaf_node_ids=[io_token_w2i.get(node,io_token_unk_idx) for node in nleaf_nodes]
        code_node_ids=[io_token_w2i.get(node,io_token_unk_idx) for node in code_nodes]
        
        code_graph={'nleaf_node_ids': nleaf_node_ids,
                    'code_node_ids':code_node_ids,
                    'nleaf_father_nleaf_base_edges': nleaf_father_nleaf_base_edges,
                    'nleaf_child_nleaf_base_edges':nleaf_child_nleaf_base_edges,
                    'code_father_nleaf_base_edges':code_father_nleaf_base_edges,
                    'nleaf_prev_nleaf_sibling_edges':nleaf_prev_nleaf_sibling_edges,
                    'nleaf_next_nleaf_sibling_edges':nleaf_next_nleaf_sibling_edges,
                    'code_prev_nleaf_sibling_edges':code_prev_nleaf_sibling_edges,
                    'code_next_nleaf_sibling_edges':code_next_nleaf_sibling_edges,
                    'nleaf_prev_nleaf_dfg_edges':nleaf_prev_nleaf_dfg_edges,
                    'nleaf_next_nleaf_dfg_edges':nleaf_next_nleaf_dfg_edges,
                    'nleaf_child_code_base_edges':nleaf_child_code_base_edges,
                    'nleaf_prev_code_sibling_edges':nleaf_prev_code_sibling_edges,
                    'nleaf_next_code_sibling_edges':nleaf_next_code_sibling_edges,
                    'code_prev_code_sibling_edges':code_prev_code_sibling_edges,
                    'code_next_code_sibling_edges':code_next_code_sibling_edges,
                    'code_prev_code_code_edges':code_prev_code_code_edges,
                    'code_next_code_code_edges':code_next_code_code_edges,
                    'code2text_map_ids': code2text_map_ids}
